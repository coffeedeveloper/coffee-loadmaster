...
var each = function (arr, cb) {
  ...
  for (var i = 0; i < len; i++) { r.push(cb(arr[i], i)); }
...
var extend = function(target) {
  ...
  for (var i = 1; i < arguments.length; i++) {
    ...
    for (var p in arg) { target[p] = arg[p]; }
  }
  return target;
};
...
var LoadMaster = (function (EventEmitter$$1) {
  ...
  if ( EventEmitter$$1 ) LoadMaster.__proto__ = EventEmitter$$1;
  LoadMaster.prototype = Object.create( EventEmitter$$1 && EventEmitter$$1.prototype );
  LoadMaster.prototype.constructor = LoadMaster;

  LoadMaster.calc = function calc$1 (eles) {
  ...
  LoadMaster.prototype._meta = function _meta () {
    if (this.opts.items !== null) {
      ...
      this.items = each(this.eles, function (el, i) { return extend({index: i}, calc(el)); });
    } else {
      this.eles = null;
      this.items = null;
    }
  ...
  LoadMaster.prototype._scroll = function _scroll (e) {
    var this$1 = this;
    ...
    if (this.items !== null) {
      ...
      if (isFast) {
        ...
        var ts = [];
        for (var i = 0; i < p; i++) {
          ts.push(this$1.lastTop + (i * this$1.opts.offset * (isForward ? 1 : -1)));
        }
        ts.push(t);
        ts.map(function (t) {
          this$1.opts.trigger.map(function (d) {
            this$1[d](t, isForward, isFast);
          });
        });
      } else {
  ...
  LoadMaster.prototype.bind = function bind () {
  ...
  LoadMaster.prototype.above = function above (top, dir, isFast) {
    ...
    if (eles.length) { this.emit('above', eles.map(function (d) { return d.el; }), dir, top, eles, isFast); }
  };

  LoadMaster.prototype.curr = function curr (top, dir, isFast) {
    var k = top + window.innerHeight;
    var eles = this.items.filter(function (d) {
      return (d.top >= top && d.top <= k) || (d.bottom >= top && d.bottom <= k);
    });

    if (eles.length) { this.emit('curr', eles.map(function (d) { return d.el; }), dir, top, eles, isFast); }
  };

  LoadMaster.prototype.below = function below (top, dir, isFast) {
    var k = top + this.opts.offset;
    var t = k + this.opts.threshold;
    var eles = this.items.filter(function (d) {
    ...
    if (eles.length) { this.emit('below', eles.map(function (d) { return d.el; }), dir, top, eles, isFast); }
  };

  LoadMaster.prototype.refresh = function refresh () {
    this._meta();
  };

  LoadMaster.prototype.off = function off () {
